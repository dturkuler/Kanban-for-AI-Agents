...
class KanbanParser {
    constructor(workspaceRoot, boardName) {
        this.boardPath = path.join(workspaceRoot, '.kanbans', boardName, '.kanbn');
    }
    async loadBoard() {
    ...
    static findFirstBoard(workspaceRoot) {
        const boardsDir = path.join(workspaceRoot, '.kanbans');
...
`;
    ...
    static deleteBoard(workspaceRoot, boardName) {
        const boardDir = path.join(workspaceRoot, '.kanbans', boardName);
        if (!fs.existsSync(boardDir)) {
